...
import { ProductDTO, CategoryDTO, CreateProductDTO, UpdateProductDTO, CreateProductSchema, UpdateProductSchema } from '@/types';
...
export default function ProductFormSheet({
  ...
  const { createProductWithImages, updateProductWithImages } = useProductMutations();
  ...
  const onSubmit = async (data: Omit<CreateProductDTO | UpdateProductDTO, 'images'>) => {
    if (images.length === 0) {
      ...
      <BottomSheetScrollView className="flex-1 px-5">
        ...
        <View className="mt-6 mb-4">
          <Button
            ...
            loading={createProductWithImages.isPending || updateProductWithImages.isPending}
